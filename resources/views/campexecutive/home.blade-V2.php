...
<style>
    ...
    body {
            background-image: url("{{ asset('homepgast/images/body-bg-900-1500.png') }}"); /* Or full URL */
            background-size: cover;        /* Makes sure image covers the whole screen */
            background-repeat: no-repeat;  /* Prevents repeating if image is smaller */
            background-position: center;   /* Centers the image */
            background-attachment: fixed;  /* Optional: makes image stay fixed while scrolling */
    ...
    .mcqsect .form-check {
        background: linear-gradient(180deg, #cccccc 10%, #ffffff);
        margin-bottom: 14px;
        padding: 8px 40px;
        width: 60%;
        border-radius: 30px;
        border: 1px solid 1px solid #898989;
    }
...
<div class="page-wrapper">
    ...
    <div class="content container-fluid">
            ...
            <div class="row">
                ...
                <div class="col-sm-4">
                    ...
                    @for ($i_mainprogress = 1; $i_mainprogress <= 15; $i_mainprogress++)
                        @php
                            $quizJson = $registered_course_detail["module_{$i_mainprogress}_quiz"] ?? null;
                            $quizData = $quizJson ? json_decode($quizJson, true) : null;
                        @endphp

                        @if (
                            $registered_course_detail["module_$i_mainprogress"]
                            && is_array($quizData)
                            && isset($quizData['mcqs'])
                            && is_array($quizData['mcqs'])
                            && count($quizData['mcqs']) > 0
                        )
                            @php
                    ...
                    @php
                        $percentageCompleted = $TotalmoduleCount_mainprogress > 0
                            ? ($moduleCompletedCount_mainprogress / $TotalmoduleCount_mainprogress) * 100
                            : 0;
                    @endphp

                    Quiz Completed:
                    <span class="text-success">
                        {{ $moduleCompletedCount_mainprogress }}/{{ $TotalmoduleCount_mainprogress }}
                    </span>
                    <span class="text-danger"> ({{ intval($percentageCompleted) }}%) </span>
                </div>
                            ...
                            <div class="col-md-12 mx-0">
                                <div id="msform">
                                    <ul id="progressbar d-none">
                                        ...
                                        @for ($i = 1; $i <= 15; $i++)
                                            @php
                                                $quizJson = $registered_course_detail["module_{$i}_quiz"] ?? null;
                                                $quizData = $quizJson ? json_decode($quizJson, true) : null;
                                            @endphp

                                            @if (
                                                $registered_course_detail["module_$i"]
                                                && is_array($quizData)
                                                && isset($quizData['mcqs'])
                                                && is_array($quizData['mcqs'])
                                                && count($quizData['mcqs']) > 0
                                                ...
                                                <li class="{{ $moduleClass }} d-none" id="account">
                                                    <strong>Quiz : {{ $i }}</strong>
                                                </li>
                                            @endif
                                        @endfor

                                        @php
                                            $moduleStyleFinalClass = ($moduleCompletedCount == $TotalmoduleCount) ? 'completed' : '';
                                        @endphp
                                        <li class="{{ $moduleStyleFinalClass }} d-none" id="confirm"><strong>Finish</strong></li>
                                    </ul>
                                                            ...
                                                            @if ($registered_course_detail["module_${i}_completed"] == 'yes')
                                                                   @if ($moduleCompletedCount == $TotalmoduleCount)
                                                                        ...
                                                                        <div class="row">
                                                                            ...
                                                                            @if (is_array($quizData) && isset($quizData['mcqs']) && is_array($quizData['mcqs']))
                                                                                @php
                                                                                    $module_pareent_mcq_count = 0;
                                                                                @endphp    

                                                                                @foreach($quizData['mcqs'] as $question)
                                                                                    @php
                                                                                        $module_pareent_mcq_count++;
                                                                                    @endphp        
                                                                                    <div class="col-md-12">
                                                                                        <div class="form-group text-left">
                                                                                            <p><strong>Question {{ $i }}:{{ $question['question'] }}</strong></p>
                                                                                        </div>
                                                                                        <div class="form-group text-left">
                                                                                            <div class="mcqsect">
                                                                                                @php
                                                                                                ...
                                                                                                @foreach($question['options'] as $option)
                                                                                                    @php
                                                                                                        $modulemcqno++;
                                                                                                        $isUserSelected = $question['user_selected_option'] == $modulemcqno;
                                                                                                        $isCorrectAnswer = $question['correct_answer'] == $modulemcqno;
                                                                                                    @endphp
                                                                                                    ...
                                                                                                    <div class="form-check">
                                                                                                        <input 
                                                                                                            ...
                                                                                                            type="radio" 
                                                                                                            name="module_{{$i}}_quiz[mcq_{{ $module_pareent_mcq_count }}_user_selected]" 
                                                                                                            id="option_{{ $module_pareent_mcq_count }}_{{ $modulemcqno }}" 
                                                                                                            value="{{ $modulemcqno }}"
                                                                                                            @if($isUserSelected) checked @endif
                                                                                                            disabled
                                                                                                        >
                                                                                                        ...
                                                                                                        <label 
                                                                                                            class="form-check-label 
                                                                                                                ...
                                                                                                                @endif"
                                                                                                            for="option_{{ $module_pareent_mcq_count }}_{{ $modulemcqno }}"
                                                                                                        >
                                                                                                            {{ $option }}
                                                                                                        </label>
                                                                                                    </div>
                                                                                                ...
                                                                                                <div class="alert alert-primary">
                                                                                                    <strong>Explanation:</strong> {{ $question['correct_answer_explanation'] }}
                                                                                                </div>
                                                                                                <div class="img brandimg text-center my-3">
                                                                                                    <img src="{{ asset('homepgast/images/brand/') }}/{{ $question['brandimage'] }}">
                                                                                                </div>
                                                                                            </div>
                                                                                        </div>
                                                                ...
                                                                <form id="mcqForm{{$i}}" action={{ route('user.submit_course_result_v2') }} method="POST">
                                                                    ...
                                                                    <div class="row">
                                                                        ...
                                                                        @if (is_array($quizData) && isset($quizData['mcqs']) && is_array($quizData['mcqs']))
                                                                                        ...
                                                                                        <div class="mcqsect">
                                                                                            ...
                                                                                            @foreach($question['options'] as $option)
                                                                                                @php
                                                                                                    $modulemcqno++;
                                                                                                @endphp
                                                                                                <div class="form-check" onclick="document.getElementById('option_{{ $module_pareent_mcq_count }}_{{ $modulemcqno }}_{{$i}}').click()">
                                                                                                    <input 
                                                                                                        class="form-check-input" 
                                                                                                        type="radio" 
                                                                                                        name="module_{{$i}}_quiz[mcq_{{ $module_pareent_mcq_count }}_user_selected]" 
                                                                                                        id="option_{{ $module_pareent_mcq_count }}_{{ $modulemcqno }}_{{$i}}" 
                                                                                                        value="{{ $modulemcqno }}" 
                                                                                                        data-analog-resid="{{ $question['correct_answer'] }}" 
                                                                                                        required
                                                                                                    >
                                                                                                    <label 
                                                                                                        class="form-check-label" 
                                                                                                        for="option_{{ $module_pareent_mcq_count }}_{{ $modulemcqno }}_{{$i}}"
                                                                                                    >
                                                                                                        {{ $option }}
                                                                                                    </label>
                                                                                                </div>
                                                                            ...
                                                                            <div class="col-md-12">
                                                                                <button type="submit" class="btn btn-block btn-primary submit-mcq" data-module="{{ $i }}">
                                                                                    Submit
                                                                                </button>
                                                                            </div>
                                                                            <div class="img brandimg text-center my-3">
                                    ...
                                    <fieldset style="{{ $moduleStyleFinal }}">
                                        @if ($moduleCompletedCount == $TotalmoduleCount)
                                         ...
                                         <a class="btn btn-danger mb-1" href="{{ route('logout') }}"
                                                ...
                                                <div class="row d-flex flex-column align-items-center">
                                                    ...
                                                    @for ($i_mainprogress = 1; $i_mainprogress <= 15; $i_mainprogress++)
                                                        ...
                                                        @if (is_array($quizData) && isset($quizData['mcqs']) && is_array($quizData['mcqs']))
                                                            @foreach($quizData['mcqs'] as $question)
                                                                @php
                                                                    if($question['user_selected_option'] == $question['correct_answer']){
                                                                            $correctAnswerCount++;
                                                                    }
                                                                @endphp
                                                            @endforeach
                                                        @endif
                                                    @endfor
                                                    <div class="corectansr">Correct answers : <strong class="text-success">{{ $correctAnswerCount }}</strong></div>
                                                    <div class="thanktxt">Thanks for your Participation</div>
                                                    <div class="thankimg">
        ...
        {{-- Feedback Form  modal --}}
        <div class="modal fade" id="feedback_form_modal___" aria-hidden="true" role="dialog">
        ...
        <div class="modal fade" id="appointment_form_modal___" aria-hidden="true" role="dialog">
            <div class="modal-dialog  modal-dialog-centered" role="document">
                <div class="modal-content">
                    <div class="modal-body">
                        @php
...
@php
    session()->forget('Module_Completed');
